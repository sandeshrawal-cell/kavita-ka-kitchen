...
export function readHousehold(storage,owner,defaults){
 ...
 return {party,confirmed:saved?.confirmed===true,jain:saved?.jain===true,exclusions:Array.isArray(saved?.exclusions)?saved.exclusions.filter(x=>typeof x==='string').slice(0,100):[],metric:saved?.metric===true};
}
